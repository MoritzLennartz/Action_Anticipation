...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for i in range(self.num_layers):
            if(i == 0):
                h = self.enc_layers[i](x, x, x, mask)            
            else:
                if((i) % 3 == 0):
                    h_prime = self.pooling_layer(h.permute(0,2,1)).permute(0,2,1)
                    h = self.enc_layers[i](h_prime, h, h, mask)
                else:
                    h = self.enc_layers[i](h, h, h, mask)

        return h  # (batch_size, input_seq_len, d_model)
